...
function beginGame() {
    ...
    for (let i = 0; i < sel_speed.length; i++) {
        ...
        if (!sel_speed[i].value) {
            createTip({ type: "error", text: "您有选手" }, 3000);
            return;
...
function moveDom(dom, obj, duration, startcallback, endcallback, movingcallback) {
    var inter = 16;
    var moveTime = Math.ceil(duration / inter);
    var changeObj = {};
    for (var prop in obj) {
        var distanceSum = obj[prop][1] - obj[prop][0];
        changeObj[prop] = distanceSum / moveTime;
    }
...
function getMaxPlayer() {
    ...
    for (var j = 0; j < speedArr.length; j++) {
        ...
        if ((Number(speedArr[j].value)) >= max) {
            max = (Number(speedArr[j].value));
            maxElement = `${j + 1}号选手获胜`;
        } else if ((Number(speedArr[j].value)) === max && (Number(speedArr[j].value)) === (Number(speedArr[j + 1].value))) {
            maxElement = "打成平手";
        }
        // console.log(maxElement.getAttribute('my-name'))
    }
    return maxElement;
}
